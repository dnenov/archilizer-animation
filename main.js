document.addEventListener("DOMContentLoaded", function () {
    ...
    const dotsSmall= createDotCluster(numSmall, ringRadius, dotGeometry, dotMaterial, sBaseOrbitSpeed, sOrbitRadius, smallDotSize); 
    const dotsLarge = createDotCluster(numLargeDots, ringRadius, dotGeometry, dotMaterial, lBaseOrbitSpeed, lOrbitRadius, largeDotSize); 
    dots.push(...dotsLarge);
    dots.push(...dotsSmall);
    ...
    function animate() {
        ...
        dots.forEach((dotData, index) => {
            ...
            dotData.orbitSpeed = THREE.MathUtils.lerp(dotData.orbitSpeed, dotData.targetOrbitSpeed, 0.005);
            dotData.orbitSize = THREE.MathUtils.lerp(dotData.orbitSize, dotData.targetOrbitSize, 0.005);

            // Step 1: Get the orbit normal (we already have it stored)
            const tangent = new THREE.Vector3().crossVectors(dotData.orbitNormal, new THREE.Vector3(0, 0, 1)).normalize();

            // Step 2: Compute the position offset along the orbit (radius * tangent)
            const orbitOffset = tangent.clone().multiplyScalar(dotData.orbitSize).negate();
    ...
    function createDotCluster(numDots, ringRadius, dotGeometry, dotMaterial, baseOrbitSpeed, orbitRadius, dotSize) {
        let clusterDots = [];
        ...
        for (let i = 0; i < numDots; i++) {
            ...
            const orbitNormal = new THREE.Vector3().crossVectors(radialDirection, new THREE.Vector3(0, 0, 1)).normalize();
    ...
    function drawOrbitNormals(scene) {
        ...
        dots.forEach((dotData) => {
            ...
            const end = start.clone().add(dotData.orbitNormal.clone().multiplyScalar(2)); // Extend by 1 unit
